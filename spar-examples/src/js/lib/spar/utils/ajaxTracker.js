define(function (require) {
	...
	function ajaxTracker(spar) {

		if (spar == null) {
			throw new Error("ajaxTracker requires a spar instance!");
		}
		...
		that.add = function (target, promise, args) {
			...
			if (globalPromise == null) {
				...
				$(spar).trigger("global.ajax.start", [triggerOptions]);
				triggerOptions.args = args;
				$(spar).trigger("ajax.start", [triggerOptions]);
		...
		function addListeners(target, globalPromiseParam, promiseParam, args) {
			...
			promiseParam.then(function (data, status, jqXHR) {
				...
				$(spar).trigger("ajax.success", [triggerOptions]);
				...
				$(spar).trigger("ajax.error", [triggerOptions]);
			});
			...
			promiseParam.always(function (dataOrjqXHR, status, errorOrjqXHR) {
				...
				$(spar).trigger("ajax.complete", [triggerOptions]);
			...
			globalPromiseParam.then(function (data, status, jqXHR) {
				...
				if (globalPromise == null || globalPromise == globalPromiseParam) {
					...
					$(spar).trigger("ajax.stop", [triggerOptions]);

					delete triggerOptions.args;
					$(spar).trigger("global.ajax.stop", [triggerOptions]);
					globalPromise = null;
					//globalPromiseArgs = {};
				} else {
