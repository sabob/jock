...
( function( global ) {
	...
	var runloop = function( circular, Hook, removeFromArray, Promise, resolveRef, TransitionManager ) {
		...
		runloop = {
			start: function( instance, returnPromise ) {
				...
				// Workaround for https://github.com/ractivejs/ractive/issues/1553
				...
				if ( instance && instance.el && promise != null) {
					// Keep track of the TransitionManager's fulfilPromise so we can manually resolve it if the DOM is destroyed before the
					// transition completes. The DOM is destroyed in Ractive$render.js.
					var o = {
						fulfil: fulfilPromise,
						instance: instance,
						tm: batch.transitionManager
					};
					( instance.el.__fulfil_transition_promises__ || ( instance.el.__fulfil_transition_promises__ = [] ) ).push( o );
					// When the promise is fulfilled, remove any transition fulfil promises registered on the Ractive element
					promise.then( function() {
						//console.log('Unrender promise resolved', instance.el['__fulfil_transition_promises__']);
						if (instance.el.__fulfil_transition_promises__.length == 0) {
							delete instance.el.__fulfil_transition_promises__;
						}
					} );
				}
	...
	var Ractive$render = function( css, Hook, getElement, log, runloop ) {
		...
		function Ractive$render( target, anchor ) {
			var this$0 = this;
			var promise, instances, transitionsEnabled, fulfilArray;
			...
			if ( !this.append && target ) {
				...
				// Workaround for https://github.com/ractivejs/ractive/issues/1553
				// If there are lingering transition fulfil promises, resolve them now before the DOM is destroyed.
				fulfilArray = target.__fulfil_transition_promises__;
				if ( fulfilArray && fulfilArray.length ) {
					fulfilTransitionPromises( fulfilArray, target );
				}
		...
		function fulfilTransitionPromises( fulfilArray, target ) {
			try {
				//var l = fulfilArray.length;
				//var clone = fulfilArray.slice(0);
				fulfilArray.splice( 0, fulfilArray.length ).forEach( function( o ) {
				//clone.splice( 0, clone.length ).forEach( function( fulfil ) {
					//console.log('Unrender promise resolved', instance.el['__fulfil_transition_promises__']);
					//o.instance.transitionsEnabled = false;
					//o.tm.cehck(o.tm);
					//return o.fulfil();
				} );
				//if (0 != fulfilArray.length) {
				//	alert("NOO	")
				//}
				//fulfilArray.splice(0, fulfilArray.length)
				//delete target.__fulfil_transition_promises__;
			} catch ( err ) {
				console.error(err);
			}
		}
