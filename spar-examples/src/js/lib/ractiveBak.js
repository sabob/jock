...
( function( global ) {
	...
	var runloop = function( circular, Hook, removeFromArray, Promise, resolveRef, TransitionManager ) {
        ...
        if (instance && instance.el) {
            var target = instance.el;
            // Keep track of the TransitionManager's fulfilPromise so we can manually resolve it if the DOM is destroyed in Ractive$render.js
            ( target.__fulfil_transition_promises__ || ( target.__fulfil_transition_promises__ = [] ) ).push( fulfilPromise );
            //instance.fulfilTransitionPromise = fulfilPromise;
        }
        // HACK end
	...
	var Ractive$render = function( css, Hook, getElement, log, runloop ) {
        ...
        // HACK start
        var fulfilArray = target.__fulfil_transition_promises__;
        if ( fulfilArray && fulfilArray.length ) {
			fulfilTransitionPromises( fulfilArray );
		}
        // HACK end
		...
		function fulfilTransitionPromises( fulfilArray ) {
			try {
				fulfilArray.splice( 0, fulfilArray.length ).forEach( function( fulfil ) {
					fulfil();
				} );
			} catch ( err ) {}
		}
	...
	var Ractive$unrender = function( css, Hook, log, Promise, removeFromArray, runloop ) {
		...
		function Ractive$unrender() {
			...
			if (this$0.el) {
				promise.then(function() {
					console.log('Unrender promise resolved', this$0.el.__fulfil_transition_promises__);
					delete this$0.el.__fulfil_transition_promises__;
					console.log('Unrender promise resolved2', this$0.el.__fulfil_transition_promises__);
				});
			}
      // HACK end
